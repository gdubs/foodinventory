...
function PrepareDataTable(TableToPopulate) {
    ...
    $(TableToPopulate).DataTable({
        ...
        "drawCallback": function (settings) {
            ...
            var exportProducts = "<input type='button' class='btn btn-sm btn-primary FoodInventoryExportProductButton' role='button' value='Export'/>"
            $(exportProducts).css('margin-left', '10px').hide().appendTo('#buttonDisplayProducts').fadeIn();

            var importProducts = "<input type='button' class='btn btn-sm btn-success FoodInventoryImportProductButton' role='button' value='Import'/>"
            $(importProducts).css('margin-left', '10px').hide().appendTo('#buttonDisplayProducts').fadeIn();
...
function InitializeEventHandlers(TableToPopulate) {
    ...
    $('.FoodInventoryExportProductButton').off().on('click', function (e) {
        e.preventDefault();
        window.location = FoodInventory.ProductsExcelURL + '/Export';
    });


    $('.FoodInventoryImportProductButton').off().on('click', function (e) {
        BootstrapDialog.show({
            type: BootstrapDialog.OK,
            title: 'Import from Excel',
            message: "<div id='containerImportForm'></div>",
            onshown: function (dialog) {
                LoadImportForm('#containerImportForm', function () {
            ...
            buttons: [
                {
                    ...
                    action: function (dialog) {
                        ...
                        //temp var files = $('#fileImportProduct')[0].files;
                        /*$.each(files, function (key, file) {
                            var formData = new FormData();
                            formData.append(file.name, file);
                            ImportExcel(formData, dialog);
                        })    */  

                        var form = $('#frmImportProduct').get(0);
...
function LoadImportForm(FormContainer, OnFormLoadedCallBack) {
    'use strict';
    ...
    var partialLoader = $.ajax({
        url: FoodInventory.ProductsImportFormURL,
        type: 'GET',
        dataType: 'html'
    });
...
function ImportExcel(formData, DialogWindow) {
    var importingExcel = $.ajax({
        url: FoodInventory.ProductsExcelURL + '/Import',
        type: 'POST',
        dataType: 'json',
        cache: false,
        data: formData,
    ...
    importingExcel.fail(function (jqXHR, textStatus, errorThrown) {
        'use strict';

        BootstrapDialog.show({
            type: BootstrapDialog.TYPE_DANGER,
            title: 'Problem with Importing Products',
            message: jqXHR.responseText
        });
    ...
    importingExcel.done(function (data) {
        'use strict';
